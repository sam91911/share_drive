...
int client_fpost(char* restrict password, uint64_t server_id, char* restrict file_name, char* upname){
	...
	for(uint64_t server_offset = 0;
		...
		if(oper_pid){
			...
			if(client_process_fpost(server_id, server_userid, self_id, oper_keyf, threshold, oper_sock, *(struct sockaddr_in*)oper_sockaddr, socket_buffer_size, password, secret, server_pubkey, pubkey_len, upname))
...
int client_fget(char* restrict password, uint64_t server_id, uint64_t call_id, char* restrict file_name, char* upname){
	...
	for(uint64_t server_offset = 0;
		...
		if(oper_pid){
			...
			if(client_login(server_id, oper_sock, socket_buffer_size, password, secret, server_pubkey, pubkey_len)) return -1;
			if(client_process_fget(server_id, server_userid, oper_sock, *(struct sockaddr_in*)oper_sockaddr, socket_buffer_size, password, secret, server_pubkey, pubkey_len, call_id, upname))	
			shutdown(oper_sock, SHUT_RDWR);
			return 0;
		}
	}
	temp_decode(file_name, 0, iv, check_hash, threshold, upname);
	return 0;
}
